...
function suit.new(theme)
	return setmetatable({
		...
		gamepadActive = false,
		gamepadPosition = 0,
		gamepadOptionsCounter = 0, -- reset each enter frame
	}, suit)
end
...
-- gamepad
function suit:registerGamepadOption(id)
	if not self.gamepadActive then
		return
	end

	self.gamepadOptionsCounter = self.gamepadOptionsCounter + 1
	if self.gamepadOptionsCounter == self.gamepadPosition then
		self:setHovered(id)
	end
end

-- Called before enterFrame, and after exitFrame
function suit:adjustGamepadPosition(adjustBy)
	if not self.gamepadActive or self.gamepadOptionsCounter == 0 then
		return
	end

	self.gamepadPosition = self.gamepadPosition + adjustBy
	while self.gamepadPosition > self.gamepadOptionsCounter do
		self.gamepadPosition = self.gamepadPosition - self.gamepadOptionsCounter
	end
	while self.gamepadPosition < 1 do
		self.gamepadPosition = self.gamepadPosition + self.gamepadOptionsCounter
	end
end

function suit:gamepadMode(mode)
	if self.gamepadActive ~= mode then
		self.gamepadActive = mode
		self.gamepadPosition = 0
	end
end

function suit:setGamepadPosition(position)
	self.gamepadPosition = position
end

-- state update
function suit:enterFrame(mouseButton)
	if not self.mouse_button_down and not self.gamepadActive then
		self.active = nil
...
function suit:exitFrame()
	...
	if self.active and self.gamepadActive then
		self.active = nil
	end
end
