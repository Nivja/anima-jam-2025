--[[
Splash has been altered to better work for the project.
]]
...
function splashlib.new(init)
  ...
  self.text = {
    obj   = love.graphics.newTextBatch(love.graphics.newFont(current_folder .. "/handy-andy.otf", 22), "made with"),
    alpha = 0
  }
  ...
  self.resize = splashlib.resize
  self.release = splashlib.release

  return self
end

-- Added code
function splashlib:resize(w, h)
  self.canvas = love.graphics.newCanvas()
end
...
function splashlib:release()
  self.canvas:release()
  self.maskshader:release()
  self.textshader:release()
  self.logoshader:release()
  self.heart.sprite:release()
  self.text.obj:release()
  self.logo.sprite:release()
  self.logo.mask:release()
end
