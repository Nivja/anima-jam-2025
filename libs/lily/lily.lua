--[[
Lily has been altered to better work for the project.
]]
...
local function multiObjectChildErrorHandler(userdata, errorMessage, source)
	...
	local multiLily = userdata[2]
	local lilyObject = multiLily.lilies[userdata[1]]
	multiLily.error(lilyObject.userdata, userdata[1], errorMessage, source)
end
...
local function multiObjectOnLoaded(info, ...)
	...
	local lilyObject = multiLily.lilies[info[1]]
	multiLily.loaded(lilyObject.userdata, info[1], select(1, ...))
...
function lily.quit()
	lily.taskChannel:performAtomic(function()
		-- Clear up the task channel
		lily.taskChannel:clear()
